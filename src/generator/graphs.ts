...
import type { ArrayGenerators } from './arrays';
import type { TreeOptions, GraphOptions } from '../types';

export interface GraphGenerators {
    tree(n: number, options?: TreeOptions): number[][];
    graph(n: number, m: number, options?: GraphOptions): number[][];
}

export function createGraphGenerators(core: GeneratorCore, arrays: ArrayGenerators): GraphGenerators {
    const generators: GraphGenerators = {
        tree(n: number, options: TreeOptions = {}): number[][] {
        ...
        graph(n: number, m: number, options: GraphOptions = {}): number[][] {
            ...
            if (type === 'tree') {
                ...
                return generators.tree(n, { oneBased, weighted });
            }
            ...
            // 普通图
                ...
                if (connected) {
                    generators.tree(n, { type: 'random', oneBased: false }).forEach(([u, v]) => addEdge(u, v));
                }
                while (edgeSet.size < m) {
