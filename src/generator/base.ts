...
import type { StringGenerators } from './strings';
...
export function createBaseGenerators(core: GeneratorCore, strings: StringGenerators): BaseGenerators {
    const generators: BaseGenerators = {
        ...
        binToHex(s: string): string {
            return generators.convert(s, 2, 16);
        },

        hexToBin(s: string): string {
            return generators.convert(s, 16, 2);
        },

        digits(len: number, radix: number): string {
            if (len <= 0) return '';
            if (radix < 2 || radix > 36) {
            ...
            return core.sample(cs.replace('0', '').split('')) + strings.string(len - 1, cs);
        },
    };
